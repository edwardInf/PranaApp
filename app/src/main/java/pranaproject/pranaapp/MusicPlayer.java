...
public class MusicPlayer {
    ...
    public static void delToPlaylist(final Context context, final long[] ids, final long playlistid) {
        final int size = ids.length;
        final ContentResolver resolver = context.getContentResolver();
        final String[] projection = new String[]{
                "max(" + "play_order" + ")",
    ...
    public static void makeDeleteItems(final long[] ids, final int offset, int len, final int base) {
        if (offset + len > ids.length) {
            len = ids.length - offset;
        }
        ...
        for (int i = 0; i < len; i++) {
            ...
            mContentValuesCache[i].remove(MediaStore.Audio.Playlists.Members.PLAY_ORDER);
            mContentValuesCache[i].remove(MediaStore.Audio.Playlists.Members.AUDIO_ID);
        }
    }
    ...
    public static final long deletePlaylist(final Context context, final String name) {
            Uri uri = MediaStore.Audio.Media.getContentUriForPath(name);
